...
const CabSection = ({ setIsBookingActive }) => {
  const [showVerify, setShowVerify] = useState(false);

  const handleBookNow = () => {
    setShowVerify(true);
    setIsBookingActive(true); // Inform parent to hide Booking Container
  };

  return (
    <div className={`page-wrapper ${showVerify ? "hide-everything-else" : ""}`}>
      {showVerify ? (
        <VerifyDet />
      ) : (
        <>
          ...
          <div className="cab-section-wrapper">
            <div className="cab-section">
              ...
              {cabData.map((cab, index) => (
                <CabCard cab={cab} key={index} onBookNow={handleBookNow} />
              ))}
